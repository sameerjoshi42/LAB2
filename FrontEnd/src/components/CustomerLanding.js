...
const CustomerLanding = () => {
    ...
    useEffect(()=>{
        const url ="http://3.128.29.95:3002/getCustLocationMongo";
    ...
    const addtoFav=(value)=>{
        const url="http://3.128.29.95:3002/addtofavMongo";
        Axios.defaults.headers.common['authorization'] = localStorage.getItem('token');
        Axios.post(url,{
    ...
    useEffect(()=>{
        Axios.defaults.headers.common['authorization'] = localStorage.getItem('token');
        const url ="http://3.128.29.95:3002/getAllRestaurantsMongo";
